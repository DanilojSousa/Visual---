	...
	<div id="corpo1">
       <?php
        ...
        echo msg_erro("Efetue o <a href='user_login.php'>Login</a> Para poder editar seus dados.");
            ...
            require "cadastro_usuario_form.php";
		 }else{
	    ...
	    if($senha1 === $senha2){
        if(empty($nome) || empty($sobrenome) || empty($email) || empty($nascimento) || empty($telefone) || empty($cidade) || empty($endereco) || empty($senha1) || empty($senha2) || empty($tipo)){
                echo msg_aviso("Todos campos são obrigatórios!!");
            }else{
			$senha = gerarhash($senha1);
			$c = "INSERT INTO cliente (nome, sobrenome, email, nascimento, telefone, cidade, uf, endereco, link, tipo, senha) values ('$nome','$sobrenome','$email','$nascimento','($form1) $form2-$form3','$cidade','$uf',
'$endereco','$link','$tipo','$senha')";
			
			if($banco->query($c)){
                 echo msg_sucesso("Usuario cadastrado com sucesso! Continuar <a class='retorno' href='user_cadastro_usuario.php'>cadastrando</a>?");
